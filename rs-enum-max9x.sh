...
declare -A camera_idx=( [a]=0 [A]=0 [b]=1 [c]=2 [d]=3 [e]=4 [E]=4 [f]=5 [g]=6 [h]=7 [i]=8 [j]=9 [k]=10 [l]=11 [G]=6 [H]=7 [I]=8 [J]=9 [K]=10 [L]=11 [M]=12 )
...
# for all d457 muxes a, b, c, d, e and f
for camera in $mux_list; do
  ...
  [[ "${camera}" == "K" || "${camera}" == "G" ]]  && vpad=6
  [[ "${camera}" == "L" || "${camera}" == "H" ]]  && vpad=1
  [[ "${camera}" == "M" || "${camera}" == "I" ]]  && vpad=7
  ...
  for sens in "${!d4xx_vc_named[@]}"; do
    ...
    [[ "${camera}" == "L" || "${camera}" == "H" ]]  && continue
    [[ "${camera}" == "M" || "${camera}" == "I" ]]  && continue
