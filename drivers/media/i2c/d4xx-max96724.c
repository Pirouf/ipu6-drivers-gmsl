...
#define MAX96724_MIPI_TX_MAP_CON(csi) (MAX96724_MIPI_TX(csi) + 0x31)
...
static int max96724_write_link(struct device *dev, u32 link)
{
	...
	*/
	...
	/* set default link to pipe selection
	*/
...
int max96724_sdev_register(struct device *dev, struct gmsl_link_ctx *g_ctx)
{
	...
	if (priv->d4xx_hacks) {
		...
		i = (unsigned int) max96724_link_to_port(priv->src_link);
		...
		if (!csi_mode_ok) {
			...
			priv->sources[i].g_ctx->csi_mode = ((priv->csi_mode == MAX96724_CSI_MODE_4X2)
                ...
                if ((g_ctx->num_csi_lanes == 2) &&
                    (priv->sources[i].g_ctx->csi_mode != GMSL_CSI_4X2_MODE)) {
                        dev_warn(dev, "%s: invalid nlanes for CSI 2X4 mode, forced to x4 nlanes\n", __func__);
                        priv->sources[i].g_ctx->num_csi_lanes = 4;
                }
	...
	for (i = 0; i < priv->max_src; i++) {
		...
		if (!priv->sources[i].g_ctx)
			continue;
		/*
		 * All sdevs should have same num-csi-lanes and phy regardless of
	...
	if (i == priv->max_src) {
		dev_err(dev,
			"%s: undefined link or csi num lanes or csi phy config mismatch\n", __func__);
		err = -EINVAL;
		goto error;
	}
				    ...
				    u8 data_type2, u32 vc_id, u32 csi_id )
{
	...
	*/
	...
	map_pipe_control[12].val = (vc_id_3b_src | vc_id_3b_dst)  << 2;
	...
	*/
	err |= MAX96724_UPDATE_BITS(priv->regmap, MAX96724_MIPI_TX_MAP_CON(csi_id),
